Python Mastery Journey: 

Welcome to my Python learning repository! 
This project tracks my progress as I learn Python from scratch using the comprehensive "Python Tutorial For Beginners in Hindi" by CodeWithHarry.

I am documenting my learning by practicing every chapter, solving practice sets, and building real-world projects to solidify my understanding of the language.
### chapters - 2 , 3  are not uploaded because didn't practise them as we already learnt while learning java and C

📚 Learning Path
I am following the structured curriculum provided in the course:

Chapter 0: Fundamentals of Programming
Chapter 1: Modules, Comments & Pip
Chapter 2: Variables & Datatypes
Chapter 3: Strings
Chapter 4: Lists & Tuples
Chapter 5: Dictionary & Sets
Chapter 6: Conditional Expressions
Chapter 7: Loops in Python
Chapter 8: Functions & Recursions
Chapter 9: File I/O
Chapter 10-11: Object-Oriented Programming (OOPs) & Inheritance
Chapter 12-13: Advanced Python concepts

🚀 Projects Included
This repository includes the following major projects:

Snake, Water, Gun Game (Chapter 8)
The Perfect Guess (Chapter 11)
Jarvis AI (Mega Project 1)
Auto-Reply AI Chatbot (Mega Project 2)
🛠️ Tools & Technologies
Language: Python 3.x
Editor: Visual Studio Code (VS Code)
Environment Management: Virtual Environments (venv)
🔗 Resource
Original Course: Python Tutorial For Beginners in Hindi by CodeWithHarry.
